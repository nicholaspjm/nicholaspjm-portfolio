...
const postersOut = resolve(root, "public/images/posters");
const manifestPath = resolve(root, "src/content/project-images.ts");
...
// Full tier serves the project detail pages. Those images display at most
// 380px tall (globals.css .gallery-item.size-l), so 2000px long edge is still
// crisp well past 2x retina while being far lighter than a near-4K master —
// the quality knob that matters at these sizes is JPEG quality, kept high.
const FULL_EDGE = 2000; // max long edge for published images
...
/**
 * Poster frame and intrinsic size for every clip.
 *
 * A row video had neither, so it rendered as a black box at the wrong width
 * until enough of the file arrived to know better: no dimensions meant no space
 * reserved, and preload="none" meant no picture. Prefetching the clips is not
 * an option, since the homepage alone references 49 of them and the library is
 * 205MB, but a poster is a few tens of kilobytes and paints immediately.
 *
 * Posters are COMMITTED rather than gitignored like the resized images. Those
 * are regenerated by sharp, which is an npm dependency and therefore always
 * present; these need ffmpeg, which is a system tool the CI runner is not
 * guaranteed to have. Committing them means a missing ffmpeg degrades to
 * "posters are not refreshed", not "the live site loses them".
 */
function videoTools() {
  try {
...
const HAS_FFMPEG = videoTools();
/**
 * Seeded from whatever the last run committed, so a machine without ffprobe
 * keeps the dimensions rather than emitting an empty map.
 *
 * The committed posters already survived a runner with no ffmpeg, but the
 * dimensions did not: they are computed here, so CI regenerated this file with
 * an empty videoDims and the deployed clips lost their reserved boxes. Derived
 * data that needs a system tool has to survive a machine that lacks it.
 */
function previousVideoDims() {
  try {
    const prev = readFileSync(manifestPath, "utf8");
    const m = prev.match(
      /videoDims: Record<string, \[number, number\]> = (\{[\s\S]*?\});/,
    );
...
const videoDims = previousVideoDims();
const videoPosters = {};
let posters = 0;

if (!HAS_FFMPEG) {
  console.warn("scan-images: ffmpeg/ffprobe not found; keeping existing video posters");
}
for (const slug of dirs(videosRoot)) {
  for (const f of files(join(videosRoot, slug))) {
    if (!VIDEO_RE.test(f) || f.startsWith(".")) continue;
    const src = join(videosRoot, slug, f);
    const url = `/videos/projects/${slug}/${f}`;
    const posterName = f.replace(/\.[^.]+$/, ".jpg");
    const posterPath = join(postersOut, slug, posterName);
    const posterUrl = `/images/posters/${slug}/${posterName}`;

    if (HAS_FFMPEG) {
      try {
        // Stream dimensions, so the box is the right shape before any of the
        ...
        if (w && h) videoDims[url] = [w, h];
      } catch {
        /* unreadable header; the clip just keeps its old behaviour */
      }
      if (!fresh(src, posterPath)) {
        try {
          mkdirSync(dirname(posterPath), { recursive: true });
          // A frame from just inside the clip: frame zero is often black on a
          // fade-in, which would defeat the point.
          execFileSync("ffmpeg", [
            "-y", "-loglevel", "error", "-ss", "0.25", "-i", src,
            "-frames:v", "1", "-vf", "scale=-2:640:flags=lanczos",
            "-q:v", "6", posterPath,
          ]);
          posters++;
        } catch {
          /* leave whatever poster is already committed */
        }
      }
    }
    if (existsSync(posterPath)) videoPosters[url] = posterUrl;
  }
}
...
export const videoDims: Record<string, [number, number]> = ${JSON.stringify(videoDims)};

/** First-frame poster per clip, so nothing renders as a black rectangle. */
export const videoPosters: Record<string, string> = ${JSON.stringify(videoPosters)};
`,
);
